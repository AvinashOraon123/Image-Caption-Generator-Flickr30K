...
word_to_index = {}
with open ("TextFiles\word_to_idx.pkl", 'rb') as file:
    word_to_index = pd.read_pickle(file, compression=None)

index_to_word = {}
with open ("TextFiles\idx_to_word.pkl", 'rb') as file:
    index_to_word = pd.read_pickle(file, compression=None)

# Derive these from your vocab dictionaries — must match training exactly
VOCAB_SIZE = len(word_to_index) + 1   # +1 for the padding/reserved index-0 slot
max_len = 80                          # matches what you already use in predict_caption
...
def build_caption_model(vocab_size, max_len):
    ...
    return Model(inputs=[inp_img_features, inp_cap], outputs=outputs)


print("Loading the model...")
model = build_caption_model(VOCAB_SIZE, max_len)
model.load_weights('model_checkpoints/model_30.h5')
